...
public class WallReflect : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Ball"))
        {
            ...
            //    {
            //        Vector3 a = Quaternion.AngleAxis(-(30 - Mathf.Acos(dot)), axisVec) * reflect;
            //        Vector3 b = Quaternion.AngleAxis((30 - Mathf.Acos(dot)), axisVec) * reflect;
    ...
    {
        if(other.gameObject.CompareTag("Ball") && gameObject == GameManager.instanse.back)
        {
            GameManager.instanse.Ball.transform.position = GameManager.instanse.BallSpawn.position;
            GameManager.instanse.Ball.directionVec = Vector3.forward;
        }
    }
}
